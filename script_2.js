...
function openDrawer(item) {
  const iconHtml = item.querySelector('.specimen-icon').innerHTML;
  const tagText = item.querySelector('.specimen-tag').textContent;
  const titleText = item.querySelector('h3').textContent;
  const descText = item.querySelector('.specimen-desc').textContent;
  const specsHtml = item.querySelector('.specimen-specs').innerHTML;

  drawerIcon.innerHTML = iconHtml;
  drawerTag.textContent = tagText;
  drawerTitle.textContent = titleText;
  drawerDesc.textContent = descText;
  drawerSpecs.innerHTML = specsHtml;
...
items.forEach(item => {
  item.addEventListener('click', () => openDrawer(item));
});
